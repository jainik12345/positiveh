...
const PortfolioPageCardInnerContactSection = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitSuccess, setSubmitSuccess] = useState(false);
  const [HotelContactDetails, setHotelContactDetails] = useState([]);
  ...
  useEffect(() => {
    const FetchPortfolioContactSection = async () => {
      try {
        const HotelNameData = await axios.get(`${BE_URL}/hotelName`);
        const FetchedHotelNameData = HotelNameData.data.data;

        const CurrentHotelName = FetchedHotelNameData.find((val) => {
          return (
        ...
        const CurrentHotelLocationData = await axios.get(
          `${BE_URL}/hotelLocation/${CurrentHotelName.id}`
        );

        if (CurrentHotelLocationData.status === 200 && CurrentHotelLocationData.data.data.length > 0) {

          if (typeof CurrentHotelLocationData.data.data === "string") {

            const tmp = JSON.parse(CurrentHotelLocationData.data.data);
            setHotelContactDetails(tmp);
            setFetchError(false);
            
          } else {

            setHotelContactDetails(CurrentHotelLocationData.data.data);
            setFetchError(false);
