...
const CareerOpportunitiesTrace = () => {
  ...
  const fetchTrashedData = async () => {
    try {
      const res = await axios.get(`${BE_URL}/careerOpportunities/trashed`);
      const formatted = res.data.data.map((item) => ({
        ...item,
        imageUrl: `${BE_URL}/Images/CareerImages/CareerOpportunities/${item.image}`,
      }));
      setData(formatted);
    } catch (err) {
  ...
  const handleRestore = async (id) => {
    try {
      await axios.patch(`${BE_URL}/careerOpportunities/restore/${id}`);
...
export default CareerOpportunitiesTrace;
