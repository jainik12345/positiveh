...
const HotelBannerBgImageTrace = () => {
  ...
  useEffect(() => {
    if (selectedHotelId) {
      axios
        .get(`${BE_URL}/hotelBannerBgImage/trashed/hotel/${selectedHotelId}`)
        .then((res) => setTrashedImages(res.data.data))
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/hotelBannerBgImage/restore/${id}`)
      .then((res) => {
        if (res.data.status === "success") {
          setTrashedImages((prev) => prev.filter((item) => item.id !== id));
  ...
  const displayedRows = trashedImages.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
      ...
      >
        <div className="flex justify-between items-center mb-7">
          <h2 className="text-xl font-bold text-[#5186c9]">
            Trashed Hotel Banner Background Image
          </h2>
          <Back onClick={() => navigate("/hotel-banner-bg-image")} />
        </div>
        ...
        >
                ...
                displayedRows.map((row, index) => (
                  ...
                  >
                    ...
                    >
                      {row.image ? (
                        <img
                          src={`${BE_URL}/Images/HotelImages/HotelBannerBg/${row.image}`}
                    ...
                    <TableCell className="text-center">
                      <Tooltip title="Restore Banner" arrow>
                        <button
          ...
          >
            <Pagination
              count={Math.ceil(trashedImages.length / rowsPerPage)}
              page={page}
