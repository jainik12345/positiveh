...
const HotelBannerBgImage = () => {
  ...
  useEffect(() => {
    if (selectedHotelId) {
      axios
        .get(`${BE_URL}/hotelBannerBgImage/hotel/${selectedHotelId}`)
        .then((res) => {
          const formatted = res.data.data.map((item) => ({
            ...item,
            image: item.image.startsWith("http")
              ? item.image
              : `${BE_URL}/Images/HotelImages/HotelBannerBg/${item.image}`,
          }));
          setRecords(formatted);
        })
        .catch((err) => console.error("Banner BG fetch failed", err));
    } else {
      setRecords([]);
    }
  }, [selectedHotelId]);
  ...
  const handleAdd = () => navigate("/hotel-banner-bg-image/insert");
  const handleTrace = () => navigate("/hotel-banner-bg-image/trace");
  const handleUpdate = (item) => {
    navigate("/hotel-banner-bg-image/update", {
      state: { bannerBgData: item },
    });
  };
  const handleDelete = (id) => {
    axios
      .delete(`${BE_URL}/hotelBannerBgImage/${id}`)
      .then((res) => {
          ...
          <Table>
            <TableHead>
              <TableRow style={{ background: "rgba(10, 20, 35, 0.89)" }}>
                {"ID,Image,Action".split(",").map((head) => (
                  <TableCell
                    key={head}
