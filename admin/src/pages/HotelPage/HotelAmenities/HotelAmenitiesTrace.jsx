...
const HotelAmenitiesTrace = () => {
  ...
  useEffect(() => {
    if (selectedHotelId) {
      axios
        .get(`${BE_URL}/hotelAmenities/trashed/hotel/${selectedHotelId}`)
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/hotelAmenities/restore/${id}`)
      .then((res) => {
      ...
      >
        <div className="flex justify-between items-center mb-7">
          <h2 className="text-xl font-bold text-[#5186c9]">
            Trashed Hotel Amenities
          </h2>
          <Back onClick={() => navigate("/hotel-amenities")} />
        </div>
          ...
          <Table>
            <TableHead>
              <TableRow style={{ background: "rgba(10, 20, 35, 0.89)" }}>
                {["ID", "Title", "Description", "Image", "Restore"].map(
                  (head) => (
                    <TableCell
                      key={head}
                  ...
                  >
                    ...
                    >
                      <div className="flex flex-wrap gap-3">
                        {row.image ? (
                          <img
                            src={`${BE_URL}/Images/HotelImages/HotelAmenities/${row.image}`}
                            alt="Amenity Preview"
...
export default HotelAmenitiesTrace;
