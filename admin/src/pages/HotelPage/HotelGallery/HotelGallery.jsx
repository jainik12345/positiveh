...
const HotelGallery = () => {
  ...
  useEffect(() => {
    if (selectedHotelId) {
      axios
        .get(`${BE_URL}/hotelGallery/hotel/${selectedHotelId}`)
        .then((res) => {
          const formatted = res.data.data.map((item) => ({
            ...item,
            image: item.image.startsWith("http")
              ? item.image
              : `${BE_URL}/Images/HotelImages/HotelGallery/${item.image}`,
          }));
  ...
  const handleAdd = () => navigate("/hotel-gallery/insert");
  const handleTrace = () => navigate("/hotel-gallery/trace");
  const handleUpdate = (item) => {
    navigate("/hotel-gallery/update", {
      state: { galleryData: item },
    });
  };
  const handleDelete = (id) => {
    axios
      .delete(`${BE_URL}/hotelGallery/${id}`)
      .then((res) => {
            ...
            <TableBody>
              {displayedRows.length === 0 ? (
                <TableRow>
                  ...
                  >
                    {selectedHotelId
                      ? "No gallery images found for this hotel"
                      : "Please select a hotel to view gallery images"}
                  </TableCell>
                </TableRow>
              ) : (
