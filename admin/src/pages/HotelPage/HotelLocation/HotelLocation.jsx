...
const HotelLocation = () => {
  ...
  useEffect(() => {
    if (selectedHotelId) {
      axios
        .get(`${BE_URL}/hotelLocation`)
        .then((res) => {
          const filtered = res.data.data.filter(
            (item) => item.hotel_id == selectedHotelId
          );
  ...
  const handleAdd = () => navigate("/hotel-location/insert");
  const handleTrace = () => navigate("/hotel-location/trace");
  const handleUpdate = (item) => {
    navigate("/hotel-location/update", {
      state: { locationData: item },
    });
  };
  const handleDelete = (id) => {
    axios
      .delete(`${BE_URL}/hotelLocation/${id}`)
      .then((res) => {
          ...
          <Table>
            <TableHead>
              <TableRow style={{ background: "rgba(10, 20, 35, 0.89)" }}>
                {["ID", "Map Link", "Address", "Email", "Phone", "Action"].map(
                  (head) => (
                    <TableCell
                      key={head}
