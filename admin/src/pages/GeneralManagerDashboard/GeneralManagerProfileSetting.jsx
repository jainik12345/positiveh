...
const GeneralManagerProfileSetting = ({ employee, hotelName }) => {
  const [form, setForm] = useState({
    name: "",
    email_id: "",
    address: "",
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.put(
        `${BE_URL}/generalManagerDataName/${employee.id}`,
        formData
      );
      setShowPopup(true);
      setTimeout(() => {
  ...
  if (!employee) return <div className="text-white">Loading profile...</div>;

  return (
    <div className="bg-[#1d1f23] text-white p-6 rounded-xl shadow-lg relative">
      <div className="flex justify-between items-start flex-col md:flex-row md:items-center md:gap-6 mb-6">
        <div className="flex-1">
          <h2 className="text-2xl mb-3 font-bold">{form.name}</h2>
          <p className="text-gray-500">
            Hotel: {hotelName || `Hotel ID: ${employee.hotel_id}`}
          </p>
        </div>
      ...
      <div className="mt-6 flex justify-end gap-4">
        {!isEditing ? (
          // <button
          //   onClick={() => setIsEditing(true)}
          //   className="bg-[#1746ff] text-white cursor-pointer px-6 py-2 rounded shadow"
          // >
          //   ✏️ Edit Profile
          // </button>
          <></>
          ...
          <>
            <button
              onClick={handleCancel}
              className="border border-[#1746ff] text-[#1746ff] px-6 py-2 rounded"
            >
              Cancel
            </button>
            <button
              onClick={handleSave}
              className="bg-white text-[#1746ff] px-6 py-2 rounded shadow"
            >
              📎 Update Change
            </button>
          </>
