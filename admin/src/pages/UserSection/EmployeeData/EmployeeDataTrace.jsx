...
const EmployeeDataTrace = () => {
  ...
  useEffect(() => {
    if (selectedHotelId) {
      axios
        .get(`${BE_URL}/employeeDataName/trashed/hotel/${selectedHotelId}`)
        .then((res) => {
          const formatted = res.data.data.map((emp) => ({
            ...emp,
            image: emp.image?.startsWith("http")
              ? emp.image
              : `${BE_URL}/Images/EmployeeDataImages/EmployeeDataName/${emp.image}`,
          }));
          setTrashedEmployees(formatted);
        })
  ...
  const handleRestore = (id) => {
    axios
      .patch(`${BE_URL}/employeeDataName/restore/${id}`)
      .then((res) => {
        if (res.data.status === "success") {
          setTrashedEmployees((prev) => prev.filter((emp) => emp.id !== id));
  ...
  const displayedRows = trashedEmployees.slice(
    (page - 1) * rowsPerPage,
    page * rowsPerPage
  );
      ...
      >
        <div className="flex justify-between items-center mb-7">
          <h2 className="text-xl font-bold text-[#5186c9]">
            Trashed Employee Data
          </h2>
          <Back onClick={() => navigate("/employee-data")} />
        </div>
        ...
        >
                ...
                displayedRows.map((row, index) => (
                  ...
                  >
                    ...
                    <TableCell className="text-center">
                      <Tooltip title="Restore Employee" arrow>
                        <button
          ...
          >
            <Pagination
              count={Math.ceil(trashedEmployees.length / rowsPerPage)}
              page={page}
...
export default EmployeeDataTrace;
