...
const ProfileSettings = ({ employee, hotelName }) => {
      ...
      try {
        ...
        if (res.data.status === "success") {
          const match = res.data.data.find(
            (d) => Number(d.id) === Number(employee.designation)
          );
          if (match) {
            setDesignationName(match.name);
          }
        }
      } catch (error) {
        console.error("Failed to fetch designation name", error);
      }
    };
  ...
  const handleSave = async () => {
    ...
    formData.append("address", form.address);
    formData.append("hotel_id", employee.hotel_id);
    formData.append("designation", employee.designation); // keep the ID
    formData.append("status", employee.status);
    formData.append("existingImage", form.existingImage);
    if (form.image) formData.append("image", form.image);
  ...
  return (
    <div className="bg-[#1d1f23] text-white p-6 rounded-xl shadow-lg relative">
      ...
      <div className="flex justify-between items-start flex-col md:flex-row md:items-center md:gap-6 mb-6">
        <div className="flex-1">
          <h2 className="text-2xl mb-3 font-bold">{employee.name}</h2>
          <p className="text-gray-400">
            Designation : {designationName || "Loading..."}
          </p>
          <p className="  text-gray-500">HotelName : {hotelName}</p>
        </div>
