...
const GeneralManagerDataName = () => {
  ...
  useEffect(() => {
    if (selectedHotelId) {
      axios
        .get(`${BE_URL}/generalManagerDataName/hotel/${selectedHotelId}`)
        .then((res) => {
          const formatted = res.data.data.map((item) => ({
            ...item,
            image: item.image.startsWith("http")
              ? item.image
              : `${BE_URL}/Images/GeneralManagerDataImages/GeneralManagerDataName/${item.image}`,
          }));
          setRecords(formatted);
        })
        .catch((err) => console.error("General Manager fetch failed", err));
    } else {
      setRecords([]);
    }
  }, [selectedHotelId]);
  ...
  const handleEdit = (row) => {
    navigate("/general-manager-data-name/update", {
      state: { generalManager: row },
    });
  };

  const handleAdd = () => {
    navigate("/general-manager-data-name/insert");
  };

  const handleDelete = async (id) => {
    try {
      await axios.delete(`${BE_URL}/generalManagerDataName/${id}`);
    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-7">
          <Trace onClick={() => navigate("/general-manager-data-name/trace")} />
          <Add
            text="Add General Manager Data"
            onClick={handleAdd}
            width="w-[270px]"
          />
        </div>
...
export default GeneralManagerDataName;
