...
/* test 1  without general manager */
...
//       otpSent = true;
//     } catch (err) {
//       // If error is 404 or "Admin not found", try employee
//       apiErrorMsg =
...
//     // If not sent to admin, try employee
//     if (!otpSent) {
//       try {
//         await axios.post(`${BE_URL}/employeeDataName/send-otp`, { email });
//         otpSent = true;
//       } catch (err) {
//         apiErrorMsg =
//           err?.response?.data?.message ||
...
//     } else {
//       setError(
//         "Database in Your EmailID is not match. Please enter correct Email ID."
//       );
//     }
//   };
...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    // Try General Manager OTP
    if (!otpSent) {
      try {
        await axios.post(`${BE_URL}/generalManagerDataName/send-otp`, {
          email,
        });
        otpSent = true;
      } catch (err) {
