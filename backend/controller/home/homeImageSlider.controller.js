...
exports.getHomeImageSliderById = (req, res) => {
  const { id } = req.params;
  db.query(
    "SELECT * FROM home_image_slider WHERE id = ? AND deleted_at = 0",
    [id],
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
...
exports.insertHomeImageSlider = (req, res) => {
  upload(req, res, (err) => {
    ...
    db.query(
      "INSERT INTO home_image_slider (image) VALUES (?)",
      [image],
      (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
        res.status(201).json({
...
exports.updateHomeImageSlider = (req, res) => {
  upload(req, res, (err) => {
    ...
    db.query(
      "UPDATE home_image_slider SET image = ? WHERE id = ? AND deleted_at = 0",
      [newImage, id],
      (err) => {
        if (err) return res.status(500).json({ error: err.message });
...
exports.restoreHomeImageSlider = (req, res) => {
  const { id } = req.params;
  db.query(
    "UPDATE home_image_slider SET deleted_at = 0 WHERE id = ?",
    [id],
    (err) => {
      if (err) return res.status(500).json({ error: err.message });
